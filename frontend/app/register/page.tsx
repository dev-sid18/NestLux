...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/register`, {
        ...
        body: JSON.stringify({ name, email, password, role: isAgent ? "agent" : "user" }),
      });
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-background p-6">
      <div className="absolute inset-0 z-0 bg-gradient-to-tr from-gold/5 to-transparent" />
      <motion.div
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-foreground/60">Join the exclusive NestLux network</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="relative">
            <User className="absolute left-4 top-3 w-5 h-5 text-foreground/40" />
            <input
              type="text"
          ...
          <div className="flex items-center gap-2 mt-4">
            ...
            />
            <label htmlFor="agent-checkbox" className="text-sm text-foreground/80 cursor-pointer">
